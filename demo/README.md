...
| **List Filtering / Ordering / Pagination**    | `blog/views.py` (`PostViewSet`)                     | The `list_post` tool advertises a `query` input mirroring the web API: filter by `category`/`author` (`filterset_fields` + `DjangoFilterBackend`), order by created/updated/title (`OrderingFilter`), and page through results (`PageNumberPagination`). |
| **Docstring Descriptions & Annotations**      | `blog/views.py` (`PostViewSet`)                     | The `PostViewSet` docstring becomes the list tool's description instead of the generated default, and read-only tools are advertised with MCP `annotations.readOnlyHint`. |
